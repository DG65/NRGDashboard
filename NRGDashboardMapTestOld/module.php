...
define('NRGDASHMAPOLD_GUID_INVERTERHUB', '{BBE2C593-1A91-426D-A714-29A9C7E87589}');
define('NRGDASHMAPOLD_GUID_METERHUB',    '{BAB8E05C-9150-43B9-9F2B-E5215FA54F0A}');
define('NRGDASHMAPOLD_GUID_CHARGERHUB',  '{9256C34E-5CFD-4F37-8BFE-E65390EBB37C}');
define('NRGDASHMAPOLD_GUID_HEISHAMON',   '{1919151A-3C0F-4C09-B906-291638EC1469}');
define('NRGDASHMAPOLD_GUID_TESSIE',      '{3F1F7E31-8BA0-4B8F-9B62-47DAD7A0B6C9}');
define('NRGDASHMAPOLD_GUID_TIBBERGRIDREWARD', '{E92F62F4-88A6-4C6E-9F0D-E76C3B1C9A01}');

class NRGDashboardMapTestOld extends IPSModule
{
    ...
    public function Create()
    {
        ...
        $this->RegisterTimer('NRGDASHMAPOLD_Refresh', 0, 'NRGDASHMAPOLD_Discover($_IPS[\'TARGET\']);');
    ...
    {
        parent::ApplyChanges();
        $this->SetTimerInterval('NRGDASHMAPOLD_Refresh', 5 * 60 * 1000);
        $this->SetVisualizationType(1);
        $this->SetStatus(102);
        $this->Discover();
    }
    ...
    public function Discover(): array
    {
        ...
        if (function_exists('HEISHA_GetFunctions')) {
            foreach (@IPS_GetInstanceListByModuleID(NRGDASHMAPOLD_GUID_HEISHAMON) as $id) {
                $key = 'heishamon_' . $id;
        ...
        if (function_exists('CHUB_GetFunctions')) {
            foreach (@IPS_GetInstanceListByModuleID(NRGDASHMAPOLD_GUID_CHARGERHUB) as $id) {
        ...
        if (function_exists('TESSIE_GetVehicleState')) {
            ...
            foreach (@IPS_GetInstanceListByModuleID(NRGDASHMAPOLD_GUID_TESSIE) as $id) {
        ...
        if (function_exists('TIBBERGR_GetPriceCurve')) {
            foreach (@IPS_GetInstanceListByModuleID(NRGDASHMAPOLD_GUID_TIBBERGRIDREWARD) as $id) {
                $key = 'tibber_' . $id;
    ...
    {
        $ids = @IPS_GetInstanceListByModuleID(NRGDASHMAPOLD_GUID_INVERTERHUB);
        return (is_array($ids) && count($ids) === 1) ? (int) $ids[0] : 0;
    }

    private function MeterHubAssignments(): array
    {
        $ids = @IPS_GetInstanceListByModuleID(NRGDASHMAPOLD_GUID_METERHUB);
