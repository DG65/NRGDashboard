...
class NRGDashboardMap extends IPSModule
{
    ...
    // Eintrag unter ihrer letzten Vergabe ('0.7.5') erhalten statt sie
        ...
        if (!@$this->ReadAttributeBoolean(self::ATTR_REVIEW_HINT_GONE)) {
            $form['elements'][] = [
                ...
                'items' => [
                    ...
                    ['type' => 'Button', 'caption' => 'Verstanden – nicht mehr anzeigen', 'onClick' => 'NRGDASHMAP_DismissReviewHint($id);'],
                ],
            ];
        }
    ...
    private const SELF_MODULE_GUID = '{B5345E99-061C-47C4-B870-0489B61BC57E}';
...
<style>
  ...
  #tourRestartBtn {
    ...
    border: 1px solid rgba(255,255,255,.4); background: rgba(20,22,26,.92);
    color: #eef2f6; font-family: {$fontCss}, system-ui; font-size: 19px; font-weight: 700; line-height: 1;
    display: flex; align-items: center; justify-content: center;
    cursor: pointer; opacity: .9; box-shadow: 0 4px 14px rgba(0,0,0,.4);
  }
  #tourRestartBtn:hover { opacity: 1; background: rgba(255,255,255,.22); }
</style>
</head>
<body>
